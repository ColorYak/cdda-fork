...
C::cell<1> render_time_cell( const W::time_data &d )
{
    if( d.text.empty() ) {
        return {};
    }
    return C::cell{ C::segment{ d.text, c_white } };
}

// === Date ===

inline constexpr const char *NF_MD_SPROUT = u8"\U000F0E66";
inline constexpr const char *NF_MD_UMBRELLA_BEACH = u8"\U000F188A";
inline constexpr const char *NF_MD_LEAF_MAPLE = u8"\U000F0C93";
inline constexpr const char *NF_MD_SNOWMAN = u8"\U000F04B7";
...
season_glyph glyph_for_season( W::season_kind s )
{
    switch( s ) {
        case W::season_kind::spring:
            return { NF_MD_SPROUT, c_green };
        case W::season_kind::summer:
            return { NF_MD_UMBRELLA_BEACH, c_yellow };
        case W::season_kind::autumn:
            return { NF_MD_LEAF_MAPLE, c_red };
        case W::season_kind::winter:
            return { NF_MD_SNOWMAN, c_light_cyan };
        case W::season_kind::unknown:
            break;
    }
    return { "", c_white };
}

C::cell<2> render_date_cell( const W::date_data &d )
{
    if( d.season == W::season_kind::unknown ) {
        return {};
    }
    const season_glyph sg = glyph_for_season( d.season );
    return C::cell{
        C::segment{ sg.glyph, sg.color },
        C::segment{ string_format( "%d", d.day ), c_light_gray },
    };
}
...
C::cell<1> render_light_cell( const W::light_data &d )
{
    const lighting_glyph lg = lighting_for_vision_mod( d.vision_mod );
    return C::cell{ C::segment{ lg.glyph, lg.color } };
}
...
C::cell<1> render_temp_cell( const W::temp_data &d )
{
    ...
    switch( d.unit ) {
        case W::temp_unit::celsius:
            glyph = NF_MD_TEMPERATURE_CELSIUS;
            break;
        case W::temp_unit::kelvin:
            glyph = NF_MD_TEMPERATURE_KELVIN;
            break;
        case W::temp_unit::fahrenheit:
            glyph = NF_MD_TEMPERATURE_FAHRENHEIT;
            break;
    }
    return C::cell{
        C::segment{ string_format( "%.0f%s", d.value, glyph ), c_light_gray },
    };
}
...
C::cell<2> render_weather_cell( const W::weather_data &d )
{
    if( d.id.empty() ) {
        return {};
    }
    ...
    if( it == weather_glyphs().end() ) {
        return C::cell{
            C::segment{},
            C::segment{ d.name, c_light_gray },
        };
    }
    const weather_glyph &wg = it->second;
    return C::cell{
        C::segment{ wg.glyph, wg.color },
        C::segment{
...
C::cell<1> render_wind_cell( const W::wind_data &d )
{
    if( !d.present ) {
        return {};
    }
...
{
    gaunthud::layout::packer p;
    p.push( render_time_cell( d.time ) );
    p.push( render_date_cell( d.date ) );
    p.push( render_light_cell( d.light ) );
    p.push( render_temp_cell( d.temp ) );
    p.push( render_weather_cell( d.weather ) );
    p.push( render_wind_cell( d.wind ) );
